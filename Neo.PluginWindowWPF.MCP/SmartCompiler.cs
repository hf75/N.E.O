...
/// <summary>
/// Embedded Roslyn compiler that runs inside the WPF PluginWindow process.
...
internal sealed class SmartCompiler
{
    ...
    private void EnsureInitialized()
    {
        ...
        // Find WPF DLLs from our own directory (this IS a WPF app, so WPF DLLs are here)
        var baseDir = AppContext.BaseDirectory;
        _additionalDlls = Directory.GetFiles(baseDir, "*.dll")
            .Where(f => !Path.GetFileName(f).StartsWith("Neo.PluginWindowWPF"))
            .ToList();
    }
}
