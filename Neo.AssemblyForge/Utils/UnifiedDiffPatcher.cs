...
{
    public sealed record ApplyResult(bool Success, string? PatchedText, string? ErrorMessage);

    public static ApplyResult TryApply(
        string originalText,
        string patchText,
        string targetFilePath,
        string? expectedClassNameForFallback = "DynamicUserControl")
    {
        ...
        var hunks = ParseHunksForTargetFile(patchNormalized, targetFilePath);

        if (hunks.Count == 0)
        {
            if (TryExtractApplyPatchAddFileReplacement(patchNormalized, targetFilePath, out var replacement))
                return new ApplyResult(true, RestoreOriginalNewLines(replacement, originalNewLine), null);

            if (LooksLikeStandaloneCSharpFile(patchNormalized, expectedClassNameForFallback))
                return new ApplyResult(true, RestoreOriginalNewLines(patchNormalized, originalNewLine), null);

            return new ApplyResult(false, null, $"Patch contains no hunks for '{targetFilePath}' (missing '@@' hunk headers or wrong patch format).");
        }
        ...
        foreach (var hunk in hunks)
        {
            ...
            if (TryApplyHunkAt(originalLines, hunk, expectedIndex, out int delta))
            {
                lineOffset += delta;
                continue;
            }
            ...
            if (fuzzyIndex >= 0 && TryApplyHunkAt(originalLines, hunk, fuzzyIndex, out delta))
            {
                lineOffset += delta;
                continue;
            }
    ...
    private static bool LooksLikeStandaloneCSharpFile(string text, string? expectedClassName)
    {
        ...
        if (!string.IsNullOrWhiteSpace(expectedClassName) &&
            trimmed.Contains($"class {expectedClassName}", StringComparison.Ordinal))
            return true;

        return trimmed.Contains("class ", StringComparison.Ordinal) &&
               trimmed.Contains("UserControl", StringComparison.Ordinal);
    }

    private static bool TryExtractApplyPatchAddFileReplacement(string patchNormalized, string targetFilePath, out string replacement)
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            if (!LineMentionsTargetFile(line, targetFilePath))
                continue;
    ...
    private static bool TryApplyHunkAt(List<string> fileLines, Hunk hunk, int startIndex, out int delta)
    {
        delta = 0;
    ...
    private static List<Hunk> ParseHunksForTargetFile(string patchNormalized, string targetFilePath)
    {
        ...
        foreach (var raw in lines)
        {
            ...
            if (line.StartsWith("diff --git ", StringComparison.Ordinal))
            {
                ...
                applyThisFile = LineMentionsTargetFile(line, targetFilePath);
                currentHunk = null;
                continue;
            }
    ...
    private static bool LineMentionsTargetFile(string line, string targetFilePath)
    {
        var lowered = line.ToLowerInvariant();
        var fileName = System.IO.Path.GetFileName(targetFilePath).ToLowerInvariant();
        return fileName.Length > 0 && lowered.Contains(fileName, StringComparison.Ordinal);
    }
