...
namespace Neo.App
{
    ...
    public class AppExportService : IAppExportService
    {
        ...
        public AppExportService(string? coreRefPath, string? desktopRefPath, IPlatformServices platformServices)
        {
            ...
            try
            {
                ...
                if( exportData.RequiresFullExport == true )
                {
                    ...
                    if (exportData.ExportSettings.UsePython == true)
                    {
                        ...
                        string pythonSource = Path.Combine(_platform.GetStartupPath(), "python", platformFolder);
                    ...
                    {
                        _platform.InjectIcon(compiledExePath, exportData.IconFullPath);
                    }
                }
        ...
        private async Task<string?> CompileExportedAppAsync(string assemblyName, string exportPath, ExportData data)
        {
            ...
            string appHostApp = data.ExportSettings.Cpe switch
            {
                CrossPlatformExport.WINDOWS => "apphost-template-windows.exe",
                CrossPlatformExport.LINUX => "apphost-template-linux",
                CrossPlatformExport.OSX => "apphost-template-osx",
                _ when OperatingSystem.IsLinux() => "apphost-template-linux",
                _ when OperatingSystem.IsMacOS() => "apphost-template-osx",
